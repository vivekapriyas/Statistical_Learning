...
misclass.tree
```
The misclassification error on the test set for this simple classification tree is `r 1 - sum(diag(misclass.tree))/sum(misclass.tree)`. 
...
```
The misclassification error on the test set for this pruned tree was found to be `r 1 - sum(diag(misclass.prune))/sum(misclass.prune)`. 
