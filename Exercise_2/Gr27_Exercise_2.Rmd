...
### a) Multiple choice 1
(i) TRUE/FALSE Påstand: Regualarizaton will reduce test error, but not training error:
...
set.seed(1)
cv.diabetes = cv.tree(tree.diabetes, K = 10, FUN = prune.misclass)
...
test.err = double(7)
for (mtry in 1:7){
  rf.fit = randomForest(diabetes~., data = d.train, mtry=mtry, ntree = 500, importance = TRUE)
  oob.err[mtry] = rf.fit$err.rate[500]
  yhat.fit = predict(rf.fit, newdata = d.test)
  misclass.fit = table(yhat.fit, response.test)
  test.err[mtry] = 1 - sum(diag(misclass.fit))/sum(misclass.fit)
}

matplot(1:mtry, cbind(test.err, oob.err), pch = 20, col = c("red", "blue"), type = "b", ylab = "...", xlab = "m")
legend("topright", legend = c("Test", "OOB"), pch = 20, col = c("red", "blue"))
...
(ii) FALSE
(iii) FALSE
(iv) FALSE ??? (får en PCA for hver kovariat? får ikke minimert informasjonsrommet?)
...
#i)
biplot(pca.GeneData,scale=0, cex=0.5)

GeneData.loading <- pca.GeneData$rotation[,1:2]
informative_loadings <- rbind(
  head(GeneData.loading[order(GeneData.loading[,1],decreasing = TRUE),]),
  head(GeneData.loading[order(GeneData.loading[,2],decreasing = TRUE),])
)

biplot(x=pca.GeneData$x[,1:2],y=informative_loadings,scale=0,cex=0.5)
```
