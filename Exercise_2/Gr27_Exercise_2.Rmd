...
```{r 1b2, echo=FALSE, eval=TRUE, fig.width=7, fig.height=3}
...
``` {r 1b3, echo=FALSE, eval=TRUE, fig.height = 4, fig.width = 6, fig.align = "center"}
...
```
```{r 1c2, echo=TRUE, eval=TRUE, fig.height=4, fig.width=5, fig.align="center"}
...
A beneficial property of the lasso regression is that predictors can be shrunken all the way to zero, meaning that parameters that is not significant for estimating the response can be completely erased from the model. In our case, the non-zero coefficients using lasso regression are ``r names(lasso.coef[lasso.coef!=0])[c(-1,-9,-10,-11,-12,-13,-14)]`` ``r names(lasso.coef[lasso.coef!=0])[c(9,10,11,12,13,14)]`` and the omitted once are ``r names(lasso.coef[lasso.coef == 0])``.

The test mean square error for the lasso regression model was found to be ``r round(MSE_lasso,2)``. 
...
```
The test mean square errors for respectively the model containing only intercept and the multiple linear regression model were found to be ``r round(MSE_ILR,2)`` and ``r round(MSE_MLR,2)``. Both of these values are greater than the ones obtained for best subset selection with BIC as model criteria and lasso regression. Hence, the models from b) and c) results in better predictions than both the full model and the model with no predictors. 
...
##### (i)
The plot below shows polynomial regression models for different degrees $d$ fitted for the ``catdat`` data using only ``daily.outdoortime`` as a covariate.

```{r 2c1,fig.height=5,fig.width=9, echo=FALSE, eval=TRUE}
...
##### (ii)
The training MSE for the models in (i) are found and shown in the table below. 

```{r 2c2,fig.height=5,fig.width=10, echo=FALSE, eval=FALSE}
...
```
The table of the training MSE show a decrease for an increasing number of degrees. There is an especially sharp decrease in the training MSE from $d=1$ to $d=2$.
...
### c) Tree-based methods to predict presence of diabetes
...
```{r 3c2, echo=FALSE, eval = TRUE, fig.height=5.5, fig.width=8, fig.align="center"}
...
misclass.tree = table(Prediction = yhat.tree, Truth =  response.test)
misclass.tree
```
The misclassification error on the test set for this simple classification tree is ``r round(1 - sum(diag(misclass.tree))/sum(misclass.tree),3)``. Furthermore, cost-complexity pruning using 10-fold cross validation was applied on the training dataset resulting in that three leaves would be optimal numbers of leaves for the pruned tree, as shown below.

```{r 3c3, echo=FALSE, eval = TRUE, fig.height=2.7, fig.width=5, fig.align="center"}
set.seed(1)
...
```{r 3c4, echo=FALSE, eval=TRUE,fig.height=2.5, fig.width=5, fig.align="center"}
...
misclass.prune
```
The misclassification error on the test set for this pruned tree was found to be ``r round(1 - sum(diag(misclass.prune))/sum(misclass.prune),3)``. Compared to the simple classification tree we see that the pruned tree have slightly lower misclassification error and classify more of the women without diabetes correctly. 

##### (ii)
Now, the classification tree will be constructed using the random forest approach. 

```{r 3c5, echo=FALSE, eval=TRUE, fig.height=3, fig.width=5, fig.align="center"}
...
##### (i)
A SVM is a more suitable approach than logistic regression for this dataset since we have $p\gg n$ and therefore always can find a separating hyperplane, excepting exact feature ties.
...
##### (iii)
Support vector machine with a linear kernel will now be used to predict whether a child that has been successfully treated against leukemia later relapsed or not. 

```{r 4b3, echo=TRUE, eval=TRUE}
svm.leukemia.linear <- svm(Category~., data = d.leukemia.train, kernel = "linear", 
                           cost = 1, scale = TRUE)
```
```{r 4b4, echo=FALSE, eval=TRUE}
#training error
...
##### (iv)
Now, the analysis will be repeated twice using the radial kernel
...
\begin{align*}
    K(\boldsymbol{x}_i, \boldsymbol{x'}_{i}) &= h_1(x_i)h_1(x'_i) + h_2(x_i)h_2(x'_i) + h_3(x_i)h_3(x'_i) + h_4(x_i)h_4(x'_i) + h_5(x_i)h_5(x'_i) + h_6(x_i)h_6(x'_i) \\
...
\begin{align*}
    ...
    h_4(x_i)h_4(x'_i) &= x_{i1}^2x_{i'1}^2 \implies h_4(x_i) = x_{i1}^2\\
    h_5(x_i)h_5(x'_i) &= 2x_{i1}x_{i2}x_{i'1}x_{i'2} \implies h_5(x_i) = \sqrt{2}x_{i1}x_{i2}\\
    h_6(x_i)h_6(x'_i) &= x_{i2}^2x_{i'2}^2 \implies h_6(x_i) = x_{i2}^2.
\end{align*}
So, $h(X) = (1, \sqrt{2}X_{1}, \sqrt{2}X_{2}, X_{1}^2, \sqrt{2}X_{1}X_{2},  X_{2}^2)$.
...
```{r 5b1, echo=TRUE, eval=TRUE, fig.align="center", fig.height=3, fig.width=4}
set.seed(1)
x1 = c(1, 2, 0, 4, 5, 6)
x2 = c(5, 4, 3, 1, 1, 2)
n = 6
...
```{r 5b2, echo=TRUE, eval=TRUE, fig.align="center", fig.height=3, fig.width=4}
...
##### (iii)
Lastly, we assign the observations new clusters based on which centroid is closest in euclidean distance and obtain the following plot.

```{r 5b3, echo=TRUE, eval=TRUE, fig.align="center", fig.height=3, fig.width=4}
for (i in 1:n){
...
```{r 5c3,echo=FALSE,eval=TRUE, fig.height=6, fig.width=9, fig.align="center"}
...
plot(CC_hclust, main = "Complete Linkage, Correlation-based", xlab = "", sub = "", cex=.9)
plot(SC_hclust, main = "Single Linkage, Correlation-based", xlab = "", sub = "", cex=.9)
plot(AC_hclust, main = "Average Linkage, Correlation-based", xlab = "", sub = "", cex=.9)
```
...
```
```{r 5d2, echo=FALSE, eval=TRUE}
TrueTissue <- c(rep("H",20),rep("D",20))
CE_2groups[CE_2groups == 1] = "H"
CE_2groups[CE_2groups == 2] = "D"

SE_2groups[SE_2groups == 1] = "H"
SE_2groups[SE_2groups == 2] = "D"

AE_2groups[AE_2groups == 1] = "H"
AE_2groups[AE_2groups == 2] = "D"

CC_2groups[CC_2groups == 1] = "H"
CC_2groups[CC_2groups == 2] = "D"

SC_2groups[SC_2groups == 1] = "H"
SC_2groups[SC_2groups == 2] = "D"

AC_2groups[AC_2groups == 1] = "H"
AC_2groups[AC_2groups == 2] = "D"

table(CE_2groups, TrueTissue)
#table(SE_2groups, TrueTissue)
#table(AE_2groups, TrueTissue)
#table(CC_2groups, TrueTissue)
#table(SC_2groups, TrueTissue)
#table(AC_2groups, TrueTissue)
```
...
##### (i)
The samples are plotted based on the first and second principal component. 

```{r 5e1, echo=TRUE, eval=TRUE, fig.align="center", fig.height=3, fig.width=3.5}
...
##### (ii)
Now, we want to determine how much of the variance in the dataset the first five principal components explain.

```{r 5e2, echo=TRUE,eval=TRUE}
