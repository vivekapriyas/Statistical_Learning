...
```
The above plot illustrates the best subsets ranked based on BIC, where the subset that minimize the BIC is at the top. Thus, the selected variables for the satisfactory model is  ``wetfood``, ``daily.playtime``, ``children.13``, ``urban``, ``bell`` and ``daily.outdoortime``.
...
Si noe mer om denne krossvalideringa. (Oppgava: Explain how you chosse lambda.)


A beneficial property of the lasso regression is that predictors can be shrinked all the way to zero, meaning that parameters that is not good(annet ord?) for estimating the response can be completely erased from the model. In our case, the non-zero coefficients using lasso regression are ``r names(lasso.coef[lasso.coef!=0])`` and the omitted once are ``r names(lasso.coef[lasso.coef == 0])``.
...
### b)
The basis functions for a cubic spline with knots at the quartiles $q_1$, $q_2$ of a variable $X$ are given as
...
From the figure showing the polynomial regression for different degrees $d =1,2,...,10$, we see that the models with  $d>1$ prove a markedly better fit for the training data points in the region $\text{Daily outdoortime} > 300$. This fits well with the training MSE being decidedly lower for those polynomial fits.
From the figure we also can see that the polynomial fits of higher degrees seem to fit the individual data points very well, which might indicate some overfitting for those models. This is especially obvious for $d=10$. However, since we are considering the training MSE, overfitting will lead to a decrease, so these models do have a lower training MSE than those with a lower degreee.
...
### b)
The length of the branches in the tree correspondes to how much the RSS decreases along that branch, i.e. the longer the branch the more reduced RSS get. When we make a regression tree we want the RSS to be as small as possible. Hence, we want to keep the branches which are the longest and the pruned tree with three leaves will have a split at r`age < 81.5` and a spilt at r`country: indonesia, japan, Korea`.
...
misclass.tree
```
The misclassification error on the test set for the simple classification tree is r`1 - sum(diag(misclass.tree))/sum(misclass.tree)`.  (Vet ikke om vi vil ha med dette, da jeg spurte om det sa hun at det var den andre feilen som ble spurt om i oppgaven, men at man sikkert kunne ha med begge...)
...
```
The misclassification error on the test set for the pruned tree is r`1 - sum(diag(misclass.prune))/sum(misclass.prune)`. 
...
```
We use random forest to construct a classification tree. When making a classification tree $m$ is chosen to be equal to $\sqrt{p}$, which in our case is $\sqrt{7} \approx 2.646 \approx 3$. Hence, we choose $m=3$. The number of trees $ntree = 500$ is not a tunig parameter and is chosen large enough. The misclassification error on the test set is r`1 - sum(diag(misclass.rf))/sum(misclass.rf)`. 

From the varians importance plot above, we can see that r`glu` and r`bmi` has the highest MeanDecreaseGini. Hence, they are the most influential predictors of diabetes. This corresponds well to the three we made in 3c) (i) where the two first splits are based on glu and bmi. 
...
```
The training misclassification error rate for the support vector classifier with radial kernel using $C=1$ and $\gamma = 10^{-2}$ is ``r 1 - sum(diag(table.leukemia.train1))/sum(table.leukemia.train1)`` and the test misclassification error rate is r`1 - sum(diag(table.leukemia.test1))/sum(table.leukemia.test1)`. 
...
```
For the support vector classifier with radial kernal using $C = 1$ and $\gamma = 10^{-5}$, the training misclassification error rate is ``r 1 - sum(diag(table.leukemia.train2))/sum(table.leukemia.train2)`` and the test misclassification error rate is ``r 1 - sum(diag(table.leukemia.test2))/sum(table.leukemia.test2)``.
